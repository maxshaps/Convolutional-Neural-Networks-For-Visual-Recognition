...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # Compute the loss
    loss = None
    scores -= np.max(scores, axis=1, keepdims=True) # Normalization trick
    probs = np.exp(scores) / np.sum(np.exp(scores), axis=1, keepdims=True)
    data_loss = np.mean(-np.log(probs[range(N), y]))
    ...
    # First, backprop into parameters W2 and b2, including L2 regularization
    grads['W2'] = np.dot(hidden.T, dscores) + reg * W2
    grads['b2'] = np.sum(dscores, axis=0, keepdims=True)

    # Next, backprop into hidden layer
    dhidden = np.dot(dscores, W2.T)

    # Next, backprop the ReLU non-linearity
    dhidden[hidden <= 0] = 0

    # Finally, backprop into W1 and b1, including L2 regularization
